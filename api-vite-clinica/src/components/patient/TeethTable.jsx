...
function TeethTable() {
  ...
  const getTeeth = async () => {
    ...
    if (datos.state == "success" && !cargando) {
      setTeeth(datos.teeth);

      console.log(datos);

      fila1 = datos.teeth.filter(
        (t) => t.letter === "c" && t.number >= 11 && t.number <= 18
      );

      fila1 = fila1.sort(function(a, b) {
        return a - b;
      });

      fila1 = fila1.reverse()

      file2 = file2 = datos.teeth.filter(
        (t) => t.letter === "c" && t.number >= 21 && t.number <= 28
      );

      file2.sort(function(a, b) {
        return a - b;
      });

    

      setTeeth([fila1, file2]);
      setLoading(false);
    }
  };
